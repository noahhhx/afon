...
/**
 * Example of how we can build a simple Counter App using Afon.
 */
class CounterTui implements Tui<Model, Msg> {

    /**
     * Initialises the Model with default values.
     * Counter at 0 & button focus on increment.
     * 
     * @return {@link Model}
     */
    @Override
    ...
    /**
     * Handling {@link Msg}'s and updating the {@link Model} accordingly.
     * <p>Focus handling for the button.</p>
     * <p>Increment/Decrement handling for the counter.</p>
     * 
     * @param model The old model
     * @param msg The Msg to update on
     * @return The new model to render
     */
    @Override
    ...
    /**
     * Building the view based on the provided {@link Model}.
     */
    @Override
    public void view(Model model, View<Msg> view) {
        view.verticalStack(
              ...
              v -> v.horizontalStack(
                    h -> h.text(""),
                    h -> h.text("Counter: " + String.valueOf(model.count())),
                    h -> h.text("")
              ),
              v -> v.horizontalStack(
                    h -> h.text(""),
              ...
              v -> v.horizontalStack(
                    h -> h.text(""),
                    h -> h.text("[tab]=next button, [enter]=select"),
                    h -> h.text("")
              ),
              v -> v.text(""),
              v -> v.text(""),
